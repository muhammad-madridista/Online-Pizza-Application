...
<div class="col-md-6">
  ...
  <strong>Coupan Updated Successfully</strong> 
  <button (click)="closeAlert()" type="button" class="close" data-dismiss="alert" aria-label="Close">
    <span aria-hidden="true">&times;</span>
  </button>
</div>
 
  <form #newCoupanForm="ngForm" autocomplete="off" novalidate (ngSubmit)="editCoupans(newCoupanForm.value)">

  
  <div class="form-group  " >
  <label for="name">Coupan Id:</label>
  <input  id="name" type="number" required class="form-control" name="coupanId"  readonly [(ngModel)]=coupan.coupanId  />  
  </div>
...
<button type="submit"  [disabled]=newCoupanForm.invalid class="btn btn-primary">Edit</button>
<button type="button"   class="btn btn-primary" (click)="cancel()">Cancel</button>
  ...
  styleUrls: ['./editcoupans.component.css']
...
export class EditCoupanComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.coupanService.getCoupanById(+this.activatedRoute.snapshot.params['cid']).subscribe(coupan => this.coupan = coupan);

  }

  editCoupans(newCoupan) {
    console.log(newCoupan)
    this.coupanService.editCoupans(newCoupan).subscribe(pipe());
    this.alert = true;
